...
```{r}
events.total <- 2 # The total number of events to generate.
tracks.total <- 3 # The total number of tracks per event.
...
for (event_id in seq(1, events.total)) {
  cluster_id <- (1 : tracks.total)
  z          <- runif(hits.total, -50, 50)
  phi        <- runif(hits.total, 0, 2*pi)
  #eta        <- runif(hits.total, 0, 2*pi)
  
  frame <- data.frame(event_id, cluster_id, phi, r=1:tracks.total
  #hits <- rbind(hits, frame)
}
hits[with(hits, order(event_id, cluster_id, r, phi, z))]
```
